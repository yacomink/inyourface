...
class EffectAnimator(Animator):
    ...
    def manipulate_frame(self, frame_image, faces, index):
        ...
        for face in faces:
            ...
            img1 = cv2.imread(os.path.dirname(os.path.realpath(__file__)) + "/../../overlays/mask.png", cv2.IMREAD_UNCHANGED);
            elements = cv2.imread(os.path.dirname(os.path.realpath(__file__)) + "/../../overlays/mask-elements.png", cv2.IMREAD_UNCHANGED);
                    
            h, status = cv2.findHomography(self.average_points, np.array(self.getPoints(face)))
